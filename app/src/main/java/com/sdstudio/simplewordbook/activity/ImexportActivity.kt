...
class ImexportActivity() : AppCompatActivity() {
            ...
            if (chooseType == 0){
                ...
                mDialog.button_add.setOnClickListener {
                    ...
                    wordBookListViewModel.insert(wordBook).invokeOnCompletion {
                        ...
                        cardList.forEach {
                            wordCardViewModel.insert(WordCard(0,it.front,it.back, wordBookId.toInt()))
                        }
                    }
    ...
    fun openWordBookList(){
        ...
        val dialogBuilder =
            AlertDialog.Builder(this).setItems(deckNameList.toTypedArray()
            ) { dialog, which ->
                button_select_wordbook.text = WordBookList[which].name
                selectedWordBookId = WordBookList[which].id
                isWordBookSelected = true
                wordCardViewModel.getCardFromBook(selectedWordBookId).observe(this, androidx.lifecycle.Observer {
